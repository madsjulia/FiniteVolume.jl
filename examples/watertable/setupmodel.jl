...
ys = coords[2, :]
zs = coords[3, :]
volumes, areasoverlengths, neighbors = FEHM.parsestor(joinpath(dirname(@__FILE__), meshdir, "wtr25.stor"))
...
zonenums, nodesinzones = FEHM.parsezone(joinpath(dirname(@__FILE__), meshdir, "wtr25_outside.zone"))
...
JLD.save("$meshdir.jld", "neighbors", neighbors, "areasoverlengths", areasoverlengths, "dirichletnodes", dirichletnodes, "rechargenodes", rechargenodes, "xs", xs, "ys", ys, "zs", zs, "topnodes", topnodes, "hycoxs", hycoxs, "hycoys", hycoys, "hycozs", hycozs, "tophyconodes", tophyconodes)
